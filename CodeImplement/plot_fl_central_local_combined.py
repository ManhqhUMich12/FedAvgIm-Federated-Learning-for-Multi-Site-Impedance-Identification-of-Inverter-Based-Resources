...
COLOR_CURVE_FED = "#4C72B0"     # old FL-vs-local curve blue
COLOR_CURVE_LOCAL = "#DD8452"   # old FL-vs-local curve orange
...
def main():
    ...
    for idx, client in enumerate(clients_gfli):
        ...
        local_histories[name] = {"train": hist_tr, "test": hist_te}
    ...
    ax.legend(loc="upper center", bbox_to_anchor=(0.5, 1.00), ncol=2, frameon=False)
    ...
    # ---------------- IBR-specific local-only vs FedAvg curves ----------------
    ...
    def plot_ibr_curve(ibr_key, ibr_label, filename):
        ...
        local_axis = np.arange(1, len(h["train"]) + 1)
        ...
        ax.plot(
            ...
            color=COLOR_CURVE_LOCAL,
            marker="o",
            markersize=3,
            markevery=max(len(h["train"]) // 10, 1),
        )
        ax.plot(
            local_axis,
            h["test"],
    ...
    if ibr1_key:
        plot_ibr_curve(ibr1_key, "IBR1", "fig_combined_local_vs_fedavg_ibr1_curves.pdf")
    if ibr9_key:
        plot_ibr_curve(ibr9_key, "IBR9", "fig_combined_local_vs_fedavg_ibr9_curves.pdf")
