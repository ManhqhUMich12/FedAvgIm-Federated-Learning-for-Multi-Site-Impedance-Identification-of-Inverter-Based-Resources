...
PLOT_CACHE_PATH = SCRIPT_DIR / "scenario_d_plot_cache.pkl"
REQUIRED_FROM_TRAINING = ("results", "curves", "TARGET_IBR", "LOCAL_TL_SOURCE_IBR")

if not all(name in globals() for name in REQUIRED_FROM_TRAINING):
    if not PLOT_CACHE_PATH.exists():
        raise RuntimeError(
            "No Scenario D plot data found. Run fl_scenario_d.py once to create "
            f"{PLOT_CACHE_PATH.name}, then run plot_scenario_d.py again."
        )
    with PLOT_CACHE_PATH.open("rb") as f:
        globals().update(pickle.load(f))
    print(f"Loaded plot cache: {PLOT_CACHE_PATH}")
...
LEGEND_COORDS = {
    ...
    "overfit_gap_per_seed": {"loc": "upper right", "bbox": (0.36, 1.0)},
