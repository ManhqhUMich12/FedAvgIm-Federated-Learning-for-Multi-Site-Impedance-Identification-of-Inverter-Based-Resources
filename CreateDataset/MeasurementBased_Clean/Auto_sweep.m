...
%% =========================================================
%  AUTO Y/Z MEASUREMENT FOR 9 GFLI CASES
...
%% ================= WHICH CASES TO RUN =================
case_list = 1;   % change to 9 if you want only one case
...
for ic = 1:numel(case_list)
    ...
    % Build timing for this case from f_eval_Hz
    timing = build_case_timing(c.f_eval_Hz, timing_cfg);

    fprintf('\n====================================================\n');
    fprintf('CASE %d | %d operating points | %d frequencies\n', ...
        case_id, size(c.ops_si,1), numel(c.f_eval_Hz));
    ...
    % ---------- Push fixed parameters to base workspace ----------
    ...
    assignin('base','Rf1',Rf1);
    assignin('base','Lf1',Lf1);
    assignin('base','Rf2',Rf2);
    assignin('base','Lf2',Lf2);
    assignin('base','Cf',Cf);

    assignin('base','Kpi',Kpi);
    assignin('base','Kii',Kii);

    assignin('base','Kppll',Kppll);
    assignin('base','Kipll',Kipll);
    ...
    meta_train = build_meta(case_id, c, base, timing, train_freq_idx, split_cfg, 'train');
    meta_test  = build_meta(case_id, c, base, timing, test_freq_idx,  split_cfg, 'test');

    Dataset = struct('X', X_train, 'Y_Y', Y_Y_train, 'Y_Z', Y_Z_train, 'meta', meta_train); %#ok<NASGU>
    save_name_train = sprintf('gfli%d_impedance_dataset_new.mat', case_id);
    ...
    save_name_test = sprintf('gfli%d_test_impedance_dataset_new.mat', case_id);
...
function meta = build_meta(case_id, c, base, timing, freq_idx, split_cfg, split_name)
    meta = struct();

    meta.description = sprintf( ...
        'GFLI%d dq-admittance/impedance dataset (features: [V2d_pu,P_pu,Q_pu,f_Hz]).', ...
        case_id);
