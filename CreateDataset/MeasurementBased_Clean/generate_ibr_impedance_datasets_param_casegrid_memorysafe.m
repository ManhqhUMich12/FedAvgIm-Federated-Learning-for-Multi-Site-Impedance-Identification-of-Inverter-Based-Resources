...
%
%  IMPORTANT:
%  - The impedance/admittance estimation idea is unchanged.
%  - Each operating point still uses:
%       1) d-axis injection -> frestimate -> Ydd, Yqd
%       2) q-axis injection -> frestimate -> Ydq, Yqq
%       3) invert Y to obtain Z
%  - The changes only reduce memory / SDI / logging accumulation.
%% =========================================================

%% ================= USER SETTINGS =================
mdl     = 'GFLI_L';              % model name only, NO .slx here
mdlFile = [mdl '.slx'];        % actual file name
axisBlk = [mdl '/AISTool/AxisSlt'];

% Use load_system instead of open_system to avoid GUI/render-process memory usage.
load_system(mdlFile);

% Model StopTime follows workspace variable Tsim
set_param(mdl, 'StopTime', 'Tsim');

% Disable unnecessary Simulink/SDI logging before the batch starts.
configure_batch_simulation(mdl);
...
%% ================= WHICH CASES TO RUN =================
case_list = 9;   % run all 9 IBRs

%% ================= MAIN LOOP =================
for ic = 1:numel(case_list)
    case_id = case_list(ic);

    % Clean previous SDI runs/logs before starting a new case.
    cleanup_simulation_memory();
    ...
    for iop = 1:Nops
        ...
        % Estimation idea is unchanged; only memory cleanup is added inside.
        ...
        % Remove OP-local results and clear SDI immediately after every OP.
        clear meas
        cleanup_simulation_memory();
    end
    ...
    save_name_train = sprintf('gfli%d_impedance_dataset.mat', case_id);
    ...
    save_name_test = sprintf('gfli%d_test_impedance_dataset.mat', case_id);
    save(save_name_test, 'Dataset', '-v7.3');
    fprintf('Saved TEST  dataset to %s\n', save_name_test);

    % Clear case-level arrays before moving to the next case.
    clear X_full Y_Y_full Y_Z_full X_train Y_Y_train Y_Z_train X_test Y_Y_test Y_Z_test Dataset
    cleanup_simulation_memory();
end
...
function p = build_ibr_parameters(case_id, p0)
    ...
    switch case_id
        ...
        case 2
            p.Kpi = 0.9 * p0.Kpi;
            p.Kii = 0.9 * p0.Kii;
            p.desc = 'Same as IBR1, but Kpi = 0.9*Kpi, Kii = 0.9*Kii';
        ...
        case 7
            ...
            p.desc = 'Same as IBR1, but Kppll = 1.1*Kppll, Kipll = 1.1*Kipll';

        case 8
            p.Kppll = 1.15 * p0.Kppll;
            p.Kipll = 1.15 * p0.Kipll;
            p.desc = 'Same as IBR1, but Kppll = 1.15*Kppll, Kipll = 1.15*Kipll';

        case 9
            % p.Kppll = 0.85 * p0.Kppll;
            % p.Kipll = 0.85 * p0.Kipll;
            % p.desc = 'Same as IBR1, but Kppll = 0.85*Kppll, Kipll = 0.85*Kipll';
            p.Rf1 =  1.5*p0.Rf1;
            p.Lf1 =  1.5*p0.Lf1;
            p.desc = 'L filter';
...
function meas = run_Y_measurement(mdl, axisBlk, fHz_meas)
    ...
    % Clean SDI before this OP. This prevents SDI disk accumulation.
    cleanup_simulation_memory();
    ...
    % Use exactly the same frestimate idea as the original code.
    sys_estim_Ydd = frestimate(SigGenDataLog_Ydd, w_meas, "rad/s");
    sys_estim_Yqd = frestimate(SigGenDataLog_Yqd, w_meas, "rad/s");

    Ydd = response_to_column(sys_estim_Ydd, Nf);
    Yqd = response_to_column(sys_estim_Yqd, Nf);

    % Free large time-domain logs before running the q-axis simulation.
    clear SigGenDataLog_Ydd SigGenDataLog_Yqd sys_estim_Ydd sys_estim_Yqd
    cleanup_simulation_memory();
    ...
    Ydq = response_to_column(sys_estim_Ydq, Nf);
    Yqq = response_to_column(sys_estim_Yqq, Nf);

    % Free large time-domain logs immediately after extraction.
    clear SigGenDataLog_Ydq SigGenDataLog_Yqq sys_estim_Ydq sys_estim_Yqq
    cleanup_simulation_memory();

    %% Build Y and Z matrices
    ...
    for k = 1:Nf
        Yk = Y(:,:,k);
        if rcond(Yk) > 1e-12
            Z(:,:,k) = Yk \ eye(2);   % same as inv(Yk), but numerically cleaner
        end
    end
    ...
    % Guarantee column vectors when Nf = 1.
    meas.Zdd = meas.Zdd(:);
    meas.Zdq = meas.Zdq(:);
    meas.Zqd = meas.Zqd(:);
    meas.Zqq = meas.Zqq(:);
...
function y = response_to_column(sys_estim, Nf)
    % Original operation: -squeeze(sys_estim.ResponseData(1,1,:))
    y = -squeeze(sys_estim.ResponseData(1,1,:));
    y = y(:);

    if numel(y) ~= Nf
        error('frestimate returned %d points, but expected %d points.', numel(y), Nf);
    end
end
...
function configure_batch_simulation(mdl)
    % Turn off logging that is not needed for the impedance calculation.
    % To Workspace blocks used by frestimate are not removed by these settings.

    cleanup_simulation_memory();

    safe_set_param(0,   'InspectSignalLogs',     'off');
    safe_set_param(0,   'ReturnWorkspaceOutputs','off');

    safe_set_param(mdl, 'SignalLogging',          'off');
    safe_set_param(mdl, 'InspectSignalLogs',      'off');
    safe_set_param(mdl, 'SaveOutput',             'off');
    safe_set_param(mdl, 'SaveTime',               'off');
    safe_set_param(mdl, 'SaveState',              'off');
    safe_set_param(mdl, 'SaveFinalState',         'off');
    safe_set_param(mdl, 'DSMLogging',             'off');
    safe_set_param(mdl, 'DataLogging',            'off');
    safe_set_param(mdl, 'ReturnWorkspaceOutputs', 'off');
    safe_set_param(mdl, 'FastRestart',            'off');
end

function cleanup_simulation_memory()
    % Clear SDI runs because SDI can accumulate large logged data on disk/RAM.
    try
        Simulink.sdi.clear;
    catch
    end

    % Disable automatic SDI archiving when this API exists in the MATLAB release.
    try
        Simulink.sdi.setAutoArchiveMode(false);
    catch
        try
            Simulink.sdi.setAutoArchiveMode('off');
        catch
        end
    end

    % Let MATLAB process cleanup events without opening/rendering new windows.
    try
...
function safe_set_param(obj, paramName, paramValue)
    % Some parameters vary by MATLAB/Simulink release.
    % Ignore unavailable parameters instead of stopping the whole dataset run.
    try
        set_param(obj, paramName, paramValue);
    catch
    end
end
